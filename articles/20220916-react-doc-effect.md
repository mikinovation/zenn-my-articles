...
副作用を引き起こすことによって同期することをあげると以下のようなものがある

- reactの状態に基づいた非Reactのコンポーネント
- サーバーとの通信
- ログの出力

等

レンダリング時やイベントハンドリング時に副作用を発生させることでReactアプリケーションは外部システムとデータを同期させることが可能である
...
propsを受け取ったりstateを管理し、また状態に応じてUIをJSXで記述することができる。またRendering codeは純粋でなければならない。HaskellやElm等純粋関数型プログラミングによる関数型言語に馴染みのある方ならご存知かもしれないが、関数は

- 関数の外側の影響を受けていない。処理が関数内に閉じていること
- 引数が同じであれば毎回同じ結果を返すこと

であることが望ましい。そして関数がこれらの状態であることを「純粋な関数である」と表現する
...
イベントハンドラーは副作用(side effects)を引き起こす。ユーザーのクリックやキーボードのタイプによってプログラムの状態が更新される
